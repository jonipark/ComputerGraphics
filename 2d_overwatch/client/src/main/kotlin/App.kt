...
public var currScore = 0
public var currLives = 10
...
class App(val canvas : HTMLCanvasElement, val overlay : HTMLDivElement) {
  ...
  fun registerEventHandlers() {
    ...
    canvas.onmousedown = { 
      event : MouseEvent ->
      scene.handleMouseDown(event.x.toFloat(), event.y.toFloat())
      currScore = scene.getScore()
      currLives = scene.getLives()
      event
    }
...
fun main() {
  ...
  // update the html overlay with the live data for the currLives, currScore

  window.setInterval({
    overlay.innerHTML = "Lives: $currLives <br> Score: $currScore"
  }, 50)
  // make overlay.innerHTML yellow color, bold, and 20px
